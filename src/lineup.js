 ...
 *
 * そこで **音だけで重なりを見つけ、位置も決める**。
 *
 * 撮影時刻は**位置の計算に一切入らない**。使うのは次の2つだけ:
 *   - 処理する順番を決める（どれから照合するか）
 *   - 音で決められなかったクリップを、どの順に後ろへ並べるか
 *
 * 照合は findOffsetFullRange（全域探索）で、撮影時刻を渡していない。
 * だから撮影時刻が録画開始でも終了でも、結果は変わらない。
 *
 * ■ 重なっているかの判定（ここが肝）
 *
 ...
 *   なので、撮影時刻では原理的に届かない。
 *
 *   そのため lineUp() は**位置の計算に撮影時刻を使わない**。
 *   ここが返す値は「どの順で照合するか」と
 *   「音で決められなかったものをどの順に並べるか」にしか使われない。
 *
 * ■ 出典（これを消さないこと）
 *
...
export async function lineUp(files, onProgress = () => {}, maxLenSec = 180,
  ...
  // 撮影時刻の順に並べる。
  //
  // ここでしか撮影時刻を使わない（処理する順番を決めるだけ）。
  // 重なりの探索は全域で行うので、この順番が多少おかしくても
  // 見つかる組は変わらない。
  const order = files
